...
本轮已在真实 PostgreSQL 16.15 和 Mailpit 上运行 API、Worker 仓储合同及核心业务闭环，并修复实际发现的问题。

| 放行范围 | 当前结论 |
| --- | --- |
| Ubuntu 核心数据库与邮件链路 | passed，具体覆盖见下表 |
| 本地代码与部署交付包 | go（本地隔离范围），最终回归与五镜像运行检查通过 |
| 目标两地完整 Release A / M5 | no-go，真实对象存储、边缘及目标环境未验收 |
| 公网开放 | no-go，最终域名、安全入口、邮件及性能门槛未完成 |

本轮证据见 [Ubuntu 验收记录](./evidence/release-a-ubuntu-acceptance-2026-09-12.md)。
...
| Python | 300 tests + 303 subtests 通过，0 skip | 已覆盖 Linux symlink 和真实回环 HTTP |
| Python 静态/API 合同 | Ruff、mypy 18 文件、OpenAPI 漂移通过 | Nginx 修改后全量 Python 已复跑通过 |
| Go test/vet/race | 最终复跑通过 | 含 Go→Python HTTP；SQL 另由上方专用库验收 |
| 前端基础验收 | 40 单元、62 工具、类型、lint、两站构建、HTTP 冒烟通过 | 布局修复后公开站重建及 HTTP 冒烟再次通过 |
...
6. 最终两站接真实 API 的浏览器业务、缓存边界、真实资料视觉、LCP/CLS、API/搜索延迟和并发负载。
7. 历史归档、目标恢复演练、版本标签一致性和完整上线执行记录。
