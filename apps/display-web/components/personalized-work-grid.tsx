...
export function PersonalizedWorkGrid({ items, emptyText }: { items: HomeItem[]; emptyText: string }) {
  ...
  const visible = useMemo(() => items.filter((item) => !hidden.has(item.id)), [hidden, items]);
  ...
  return <div aria-busy={preferenceState === "loading" || busyWorkID !== null}>
    {visible.length ? <div className="work-grid">{visible.map((item, index) => <WorkCard key={item.id} item={item} position={index + 1} onHide={preferenceState === "ready" ? hide : undefined} />)}</div> : <div className="empty-copy">{emptyText}</div>}
